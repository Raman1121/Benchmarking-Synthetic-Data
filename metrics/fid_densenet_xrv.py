...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image = skimage.io.imread(img_path).T
        if self.transform:
            image = self.transform(image)
            image = xrv.datasets.normalize(image.numpy(), 255)
...
def main(args):
    ...
    real_dataset = ImageDataset(
        img_paths=real_image_paths, transform=transform)
    ...
    # FID
    fid = FrechetInceptionDistance(feature=fe).to(device)
    ...
    # KID
    kid = KernelInceptionDistance(subset_size=KID_SUBSET_SIZE, feature=fe).to(device)
    ...
    # Save results
    results = {
        "FID (DenseNet)": round(fid_value.item(), 3),
        "KID (DenseNet)": round(kid_mean.item(), 3),
    ...
    def prepare_savename(args):
        if args.experiment_type == 'conditional':
            savename = "conditional_image_generation_metrics_densenet.csv"
        else:
            if(args.num_shards is not None):
                savename = f"image_generation_metrics_shard_{args.shard}_densenet.csv"
            else:
